...
class NearestNeighborRetriever(Model):
    def __init__(
        ...
        #### OLD VERSION HERE #### Doesn't support multiple-word spans
        '''
        ...
        #### NEW VERSION HERE #### Supports multiple-word spans
        self.target_dataset_embeddings = torch.stack(
            tuple(torch.tensor(inst["span_embeddings"].array.mean(axis=0)) for inst in target_dataset)
        ).to(device)  # Average embeddings on multi-word spans
        #### NEW VERSION HERE ####
        ...
        self.dummy_param = torch.nn.parameter.Parameter(torch.tensor([1.0]))
    ...
    def forward(
        ...
        target_word_embeddings = [embedded_text[i][label_span[i][0].item()] for i in range(len(label_span))]
        ...
        # Get the query embedding: in the general case, we have n words in context, and we take their average pool
        # Allows multi-word spans by averaging their token embeddings.
        target_word_embeddings = []
        for i in range(len(label_span)):
            span_start = label_span[i][0].item()
            span_end = label_span[i][1].item()
            span_embedding = torch.mean(embedded_text[i][span_start : span_end + 1], dim=0)  # Average over span for multi-word anchors
            target_word_embeddings.append(span_embedding)
        query_embedding = torch.stack(target_word_embeddings, 0) # Stack the embeddings for batch processing
        query_embedding = query_embedding.reshape((1, -1))
        #### NEW VERSION HERE ####
        ...
        top_n_results = list(zip(top_indices, top_distances))
...
class NearestNeighborPredictor(Predictor):
    def predict(self, sentence: List[str], label_span_start: int, label_span_end: int, label: str) -> JsonDict:
...
class RandomRetriever(Model):
    ...
    def __init__(
        ...
        self.target_dataset_embeddings = torch.stack(
            tuple(torch.tensor(inst["span_embeddings"].array.mean(axis=0)) for inst in target_dataset)
        ).to(device) # Average over multi-word spans
         #### NEW VERSION HERE ####
