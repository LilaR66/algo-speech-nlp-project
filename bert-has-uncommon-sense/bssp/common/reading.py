...
from allennlp.data.token_indexers import PretrainedTransformerIndexer ### NEW ###
...
def make_indexer(cfg):
    ...
    if "camembert" in cfg.embedding_model:
        return PretrainedTransformerIndexer(cfg.embedding_model)
    elif cfg.is_transformer():
        return PretrainedTransformerMismatchedIndexer(cfg.embedding_model, namespace="tokens")
    else:
        return SingleIdTokenIndexer(namespace="tokens")
 #### NEW VERISON HERE ####
...
def activate_bert_layers(embedder, bert_layers):
    ...
    """
    # whew!
    #scalar_mix = embedder.token_embedder_tokens._matched_embedder._scalar_mix.scalar_parameters ### OLD ###

    #### NEW HERE ####
    if hasattr(embedder.token_embedder_tokens, "_matched_embedder"):
        scalar_mix = embedder.token_embedder_tokens._matched_embedder._scalar_mix.scalar_parameters
    else:
        scalar_mix = embedder.token_embedder_tokens._scalar_mix.scalar_parameters
    #### NEW HERE ####
...
def make_embedder(cfg):
    ...
    if cfg.bert_layers is not None:
        ...
        if "camembert" in cfg.embedding_model:
            token_embedder = PretrainedTransformerEmbedder(model_name=cfg.embedding_model, last_layer_only=False)
        else:
            token_embedder = PretrainedTransformerMismatchedEmbedder(model_name=cfg.embedding_model, last_layer_only=False)
